...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border bg-white px-5 py-4" style={{ borderColor: "var(--color-ui-border)" }}>
      <p className="text-xs font-semibold uppercase tracking-wide text-ui-muted">{label}</p>
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {(data?.screens ?? []).map((s) => (
          <div key={s.id} className="rounded-md border bg-white p-4" style={{ borderColor: "var(--color-ui-border)" }}>
            ...
            <p className="mt-2 text-xs text-ui-muted">{s.code}</p>
          </div>
        ))}
        {data && data.screens.length === 0 && (
          <p className="text-sm text-ui-muted">No hay pantallas registradas.</p>
        )}
      </div>
    </div>
  );
