        ...
        <>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <section
              ...
              style={{ borderColor: "var(--color-panel-border)" }}
            >
              <h2 className="text-sm font-bold text-panel-ink">Orden de reproducción</h2>
              {items.length === 0 ? (
                ...
                <ol className="mt-3 space-y-2">
                  {items.map((it, i) => (
                    <li
                      ...
                      style={{ borderColor: "var(--color-panel-border)" }}
                    >
                      <span className="w-6 text-sm font-bold text-panel-muted">
                        {i + 1}
                      </span>
                      ...
                      />
                      <span className="text-xs text-panel-muted">s</span>
                      <button
                        onClick={() => run(() => movePlaylistItem(playlist.id, it.id, "up"))}
                        disabled={pending || i === 0}
                        className="px-1.5 text-panel-muted disabled:opacity-30"
                        aria-label="Subir"
                      >
                        ▲
                      </button>
            ...
            >
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-bold text-panel-ink">Contenidos disponibles</h2>
                <button
                  ...
                  {available.map((c) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-panel-muted">{c.label}</p>
                      </div>
                      <button
